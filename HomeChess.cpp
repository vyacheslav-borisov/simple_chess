...
DWORD WINAPI AIThread(LPVOID lpParameter)
{
    ...
    TurnGenerator* pTurnGen = new TurnGenerator(pList);
    
    TurnInfo turn;
    bool bRes = pTurnGen->generateTurn(turn);
    if(bRes)
    {
...
HomeChessApllication::loadResources()
{
    ...
    HMODULE hLib = LoadLibrary(HOMECHESS_MODULE_PATH);
    if(hLib == NULL)
    {
        return false;
    }
    ...
    for(int i = 0; i < 37; i++)
    {
        ...
        m_TextureTable.insert(make_pair<int, LPDIRECT3DTEXTURE9>(m_TexIdentifiers[i], pTex));        
    }

    LPDIRECT3DTEXTURE9 pGameBoardTexture = NULL;
    hr = D3DXCreateTextureFromResource(device, hLib, MAKEINTRESOURCE(HOMECHESS_BOARD), &pGameBoardTexture);
	if(FAILED(hr))
	{
		return false;
	}
        
    m_TextureTable.insert(make_pair<int, LPDIRECT3DTEXTURE9>(HOMECHESS_BOARD, pGameBoardTexture));
...
int APIENTRY _tWinMain(HINSTANCE hInstance,
                     ...
                     int       nCmdShow)
{
	HomeChessApllication app(true, WndProc);
    if(!app.create())
	{
		return FALSE;
	}
