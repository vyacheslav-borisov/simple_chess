...
{
 //прежде чем выбирать ход-побег, нужно просмотреть список угрожающих фигур
 HOTPOINT& hp = mWhiteHP.getHotPoint(shape->getHCell(), shape->getVCell());
 if(hotPointIsEmpty(hp))
 {
     //фигура не под угрозой, выходим
     return false;
 }
 for(HOTPOINT_ITERATOR it = hp.begin();
	 it!= hp.end(); ++it)
 {
     //проверяем угрожающие фигуры:
     //если среди них есть ладья, ферзь или слон
     //мы должны проверить, не закрываем ли мы от них фигуру
     //более высого ранга
     TurnInfo info = *it;
     if((info.src == CT_SLON) || (info.src == CT_LADYA) || (info.src == CT_FERZ))
     {
         ...
         //нужно для предотвращения бесконечного цикла
         //а то мало ли что....
         ...
         while(TRUE)
         {
             ...
             if(pShape != NULL)
             {
                 ...
                 {
                     //прикрываемая фигура более высокого ранга - выходим
                     return false;
                 }else
                 {
                     //имеет смысл бежать - прерываем основной цикл
                     bContinue = false;
                 }
             }
             i++;
 ...
 //для начала попробуем выбрать из аткакующих фигур
 ...
 //если нет возможности атаковать - выбираем из нейтральных ходов
 if(mNeitralTurns.size())
 {
...
/*
 ход щит имеет смысл, если угрожающая фигура одна и она являеться
 ладьей ферзем или слоном 
*/
bool 
TurnGenerator::chooseWardTurn(ChessShape* shape, TurnInfo& turn)
{
    ...
    while(TRUE)
    {
        ...
        for(HOTPOINT_ITERATOR it = bhp.begin(); it != bhp.end(); ++it)
        {
            ...
            //спекциальный случай - пешки не могут сбить фигуру по прямой
